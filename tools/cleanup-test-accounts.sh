...
#
# Adapted from LC-rs2-r377 scripts/cleanup-test-accounts.sh for rs2b0t tools
# that hit a always-on local Server (LOGIN_SERVER=false → file saves).
#
# Default players dir (override with --engine or SERVER_ENGINE / LC_ENGINE):
#   ../Server/engine/data/players/<profile>/
# or
#   $SERVER_ENGINE/data/players/<profile>/
#
...
#
# Ops notes:
#   - Prefer logout / idle engine before --apply (do not wipe a logged-in .sav mid-suite).
#   - Stagger multi-suite boots; fresh names still leave .sav forever until this script.
#   - KEEP protects basenames you care about even with --all-saves.
#
...
# Prefer sibling Server checkout (common local layout), then env, then vendor.
...
# Generated by tools/* live harnesses (prefix + base36 stamp, ≤12 chars).
# Keep this list in sync when adding a new default USER_NAME / username prefix.
# Nav fleets: nvtr (travel), nv2r (routes), nv2s (stress), n2rg / gbrp (gather path).
# Do not use 1-char prefixes — too easy to match real accounts.
DEFAULT_PREFIXES="
